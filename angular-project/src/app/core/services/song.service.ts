//service to manage songs (crud logic!!!)
...
export class SongService {
  ...
  addSong(
    ...
    this.songs.update((s) => [
      ...s,
      { id: id, title: t, artist: art, album: alb, genre: g, year: y },
    ]);
  ...
  updateSong(
    ...
    this.songs.update((songList) =>
      songList.map((s) =>
        s.id === newId
          ? { ...s, title: t, artist: art, album: alb, genre: g, year: y }
          : s
      )
    );
  }
  ...
  removeSong(songName: string) {
    this.songs.update((songList) =>
      songList.filter((s) => s.title !== songName)
    );
